...
{{-- Page Title --}}
@section('page-title')
{{ $elc->elc_name }} <small>Edit Position</small>
@endsection
...
<div class="row">
    <div class="col-sm-12">
        <div id="add-position-whirl" class="card-box table-responsive">
            <h4 class="m-t-0 header-title"><b>Edit Position</b></h4>
            <p class="text-muted font-13 m-b-30">
                Please fill up the form with correct responding value
            </p>
            <form id="edit-position" class="form-horizontal">
                @csrf
                        <div class="form-group">
                            <label class="col-md-2 control-label">Position Name</label>
                            <div class="col-md-5">
                                <input type="hidden" id="elc-id" value="{{ $elc->id }}">
                                <input type="text" id="pos-name" class="form-control" value="{{ $position->position_name }}" placeholder="Position Name">
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-5">
                               <select id="pos-type" class="form-control">
                                    ...
                                    <option value="1" @if($position->type == 1) selected @endif>All</option>
                                    <option value="2" @if($position->type == 2) selected @endif>Department Select</option>
                                    <option value="3" @if($position->type == 3) selected @endif>Year Select</option>
                                </select>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-md-5">
                               <input  id="pos-max" type="number" class="form-control" value="{{ $position->max }}" placeholder="Choice">
                            </div>
                        </div>
...
$('#edit-position').submit(function(e){
    ...
    var max = $('#pos-max').val();
    $.ajax({
        url: "/admin/election/position/update/{{ $position->id }}",
        type: 'POST',
        dataType: 'json',
        data:{
            '_token' : token,
        ...
        {   
            ...
            {
                toastr['success'](name+" position has been updated.");
                window.location = '/admin/election/precinct/show/{{ $elc->id }}';
            }
        },
        beforeSend: function(){
