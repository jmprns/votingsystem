...
<div class="row">
    <div id="add-candidate-whirl" class="col-lg-9">
            ...
            <div class="ibox-content">
                <form id="add-candidate-form" method="POST" class="form-horizontal">
                    ...
                    <div class="form-group"><label class="col-sm-2 control-label">Full Name</label>
                        <div class="col-sm-10">
                            <div class="row">
                                @php($name = explode('__', $candidate->info->name))
                                ...
                                <div class="col-md-4">
                                    <select data-placeholder="Choose Position" id="position-select" class="form-control" style="width:100%;" tabindex="4" required>
                                        ...
                                        @foreach($positions as $position)
                                            <option @if($candidate->position_id == $position->id) selected @endif value="{{ $position->id }}">{{ $position->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="col-md-4">
                                    <select data-placeholder="Choose Party" id="party-select" class="form-control" style="width:100%;" tabindex="4" required>
                                        ...
                                        @foreach($parties as $party)
                                            <option @if($candidate->party_id == $party->id) selected @endif value="{{ $party->id }}">{{ $party->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                    ...
                    <div class="form-group"><label class="col-sm-2 control-label">Course - Year</label>
                        <div class="col-sm-10">
                            <div class="row">
                                <div class="col-md-8">
                                    <input type="text" class="form-control" readonly value="{{ $candidate->info->course->name." - ".$candidate->info->year->name }}">
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        <div class="col-sm-4 col-sm-offset-2">
                            ...
                            <button class="btn btn-primary" type="submit">Update Candidate</button>
                        </div>
                    </div>
    ...
    <div id="add-candidate-whirl" class="col-lg-3">
        <div class="ibox float-e-margins">
            <div class="ibox-content">
                <img id="cand-image" src="{{ asset('img/candidates') }}/{{ $candidate->image }}" class="image-responsive center-block">
                <br>
                <h4 align="center">(Candidate Image)</h4>
            </div>
        </div>
    </div>
</div>
...
$('#add-candidate-form').submit(function(e){
    ...
    $.ajax({
        url: "/election/candidate/update/{{ $candidate->id }}",
        type: 'POST',
        dataType: 'json',
        data:{
